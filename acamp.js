...
//esse é massa
function AddNovoObjeto(numberInList, objCalling) {
    


  let objCall = objCalling[12].obj;
  let objReal = eval(objCall); // Obtém o objeto real a partir da string
  ...
  if (objReal) { // Verifica se o objeto existe
    ...
    invAcamp[findFirstTerm(invAcamp, objCalling)][12].quantia--


    if (invAcamp[findFirstTerm(invAcamp, objCalling)][12].quantia == 0)
    {

      invAcamp.splice(numberInList, 1);
      document.getElementById('acampDivPlace').removeChild(document.getElementById("acampInv" + objCalling[0]))
...
function AddObjAcampInv(item){

    if (findFirstTerm(invAcamp, item) == -1)
    {

    invAcamp.push(item)
    let pushListNumber = findFirstTerm(invAcamp, item);

    invAcamp[pushListNumber][12].quantia = 1;

    let AcampInv = document.getElementById("acampDivPlace");
    const InvBlock = document.createElement("button");
    AcampInv.appendChild(InvBlock);
    InvBlock.classList.add("blockInvAcamp");
    
    const InvBlockP = document.createElement("p");
    InvBlock.appendChild(InvBlockP);

    InvBlockP.innerHTML = item[0];

    InvBlock.id =  "acampInv" + item[0];


      // Immediately Invoked Function Expression (IIFE) to create a new scope
      (function(pushListNumber, item) { 
        InvBlock.onclick = function() {

            AddNovoObjeto(pushListNumber, item); 

        };
      })(pushListNumber, item); // Pass the current pushList value to the IIFE
    }

    else {

    invAcamp[findFirstTerm(invAcamp, item)][12].quantia++
